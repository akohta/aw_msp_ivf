...
int main()
{
  ...
  if(ms.aw.n_pw!=1 || ms.aw.n_bb!=0 || ms.aw.n_fb!=0){
    printf("This program is available for single plane wave. Exit\n");
    exit(1);
  }
  free_amsp(&ms);
  ...
  if((fp=fopen("radiation_force1.txt","wt"))==NULL){    printf("Can not open the file.\n");    exit(1);  }
  ...
  for(j=0;j<nj;j++){
    ...
    for(i=0;i<ni;i++){
      ...
      ms.aw.bd.pw[0].f=f; // reset frequency
