...
void p_field_x(IMD *id,AMSP *ms);
void p_field_y(IMD *id,AMSP *ms);
void p_field_z(IMD *id,AMSP *ms);
void output_field(char *pl,IMD *id,AMSP *ms);
...
int main(int argc,char *argv[])
{
  ...
  // x=0 plane
  p_field_x(&id,&ms);
  output_field("yz",&id,&ms);
  // y=0 plane
  p_field_y(&id,&ms);
  output_field("xz",&id,&ms);
  // z=0 plane
  p_field_z(&id,&ms);
  output_field("xy",&id,&ms);
...
void p_field_x(IMD *id,AMSP *ms)
{
  ...
  for(i=0;i<id->m;i++){
    ...
    for(j=0;j<id->m;j++){
      ...
      total_pv_amsp(&p,v,x,id->type,ms); // total field
...
void output_field(char *pl,IMD *id,AMSP *ms)
{
  ...
  dt=1.0/(ms->aw.f*(double)id->ts);
  
  #pragma omp parallel for schedule(dynamic) 
  for(n=0;n<id->ts;n++){
    output_png(n,cexp(-I*2.0*M_PI*ms->aw.f*dt*(double)n),pl,id);
  }
